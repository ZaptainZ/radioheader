...
if echo "$FILE_PATH" | grep -q "/memory/"; then
  jq -n '{
    hookSpecificOutput: {
      ...
      additionalContext: "📡 RadioHeader Echo triggered: You just updated a project memory file. Perform these checks (mandatory):\n\n1. **Global Echo**: Is this experience useful across projects? If yes → update ~/.claude/radioheader/topics/ with [source:ProjectName] tag. Create the topic file and update INDEX.md if needed.\n2. **Project docs sync**: Did this work change project architecture, key paths, or tech stack? If yes → update project overview doc (typically projectBasicInfo/01_PROJECT_OVERVIEW.md).\n3. **Task log**: Did you complete a significant task (bug fix, feature, architecture change)? If yes → write to logs directory (typically projectBasicInfo/logs/YYYY-MM-DD-topic-<role>.md — use cc for Claude Code, codex for Codex, or your initials if you are a human; do NOT default to cc if you are not Claude Code).\n\nCheck all three items. Briefly state which Echo actions you performed."
    }
  }'
